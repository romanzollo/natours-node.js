...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if (bookBtn) bookBtn.addEventListener('click', async (e)=>{
        ...
        try {
            const res = await (0, _axiosDefault.default).post(`/api/v1/bookings/checkout-session/${tourId}`);
            const confirmationUrl = res.data?.session?.confirmation_url;
            if (!confirmationUrl) throw new Error('YooKassa confirmation URL was not returned by API.');
            ...
            if (res.data?.session?.id) localStorage.setItem('pendingYooKassaPaymentId', res.data.session.id);
